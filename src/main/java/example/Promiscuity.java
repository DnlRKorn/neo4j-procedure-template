...
public class Promiscuity {
    ...
    public Stream<PathOutput> promiscuityPath(
            ...
            @Name("tailNode") Node tailNode,
            @Name("k") Number k_input) {
        ArrayList<PathOutput> result = new ArrayList<>();
        int k = k_input.intValue();
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (x != -1) {
                ...
                result.add(new PathOutput(best_score, p));
            }
        }
    ...
    private Path buildPath(PathEntry head, Node tail) {
        ...
        for(Node n = head.node; n!=null; head=head.parent){
            nodeArrayList.add(n);
        }
        ...
        while(nodeIterator.hasNext()){
            ...
            builder.push(getRelationship(currentPathEnd,nextAdditionToPath));
